...
export function TweetCard({ item, metadata }: TweetCardProps) {
  ...
  if (!author) {
    console.warn("[TweetCard] No author found for content:", item.id)
    return null
  }
  ...
  return (
    <article
      ...
      onDragStart={(event) => {
        ...
        event.dataTransfer?.setData("application/x-content-title", `@${author.handle}`)
        event.dataTransfer?.setData("text/plain", tweetText)
        event.dataTransfer?.setDragImage(createDragImage(`@${author.handle}`), 0, 0)
      }}
    >
      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1 overflow-hidden">
          ...
          <div className="flex items-start gap-1">
            ...
            <span className="truncate text-sm text-slate-500">@{author.handle}</span>
          </div>
