...
MOCK_PLUGINS = [
    ...
    {"data_source": "medium", "title": "Medium Blog", "credentials": {}, "properties": {"platform": "medium"}},
    {"data_source": "notion", "title": "Notion Workspace", "credentials": {}, "properties": {"platform": "notion"}},
    {"data_source": "github", "title": "GitHub Repository", "credentials": {}, "properties": {"platform": "github"}},
    {"data_source": "linkedin", "title": "LinkedIn Profile", "credentials": {}, "properties": {"platform": "linkedin"}},
]
...
def api_request(
    method: str, url: str, headers: dict[str, str], json_data: dict[str, Any] | None = None, timeout: int = 10
) -> tuple[bool, Any]:
...
def create_plugins(api_url: str, auth_token: str) -> list[dict[str, Any]]:
    ...
    for i, plugin_data in enumerate(MOCK_PLUGINS, 1):
        ...
        success, result = api_request("POST", f"{api_url}/api/v1/plugins", headers, plugin_data)
...
def create_sources(api_url: str, auth_token: str, plugins: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for i, source_data in enumerate(MOCK_SOURCES, 1):
        ...
        success, result = api_request("POST", f"{api_url}/api/v1/sources", headers, source_data)
...
def create_content(api_url: str, auth_token: str, sources: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for i, content_data in enumerate(MOCK_CONTENT, 1):
        ...
        success, result = api_request("POST", f"{api_url}/api/v1/content", headers, content_data)
...
def main():
    ...
    parser.add_argument("--token", default=os.getenv("AUTH_TOKEN"), help="Authentication token (default: $AUTH_TOKEN)")
